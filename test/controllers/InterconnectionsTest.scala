...
class InterconnectionsTest extends PdgSpec with MockitoSugar with Results {
  ...
  "Interconnection controller" must {
    ...
    "delete Profile ok" in {
      ...
      when(interconnectionService.receiveDeleteProfile("AR-C-SHDG-1190", DeletedMotive("ahierro", "motivo", 2),"SHDG","SHDG",true, "user")).thenReturn(Future.successful(Right(())))
      val requestObj: JsValue = Json.parse(
        """
    ...
    "delete Profile no ok" in {
      ...
      when(interconnectionService.receiveDeleteProfile("AR-C-SHDG-1190", DeletedMotive("ahierro", "motivo", 2),"","", true, "user")).thenReturn(Future.successful(Right(())))
      val requestObj: JsValue = Json.parse(
        """
