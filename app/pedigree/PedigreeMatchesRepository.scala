...
class MongoPedigreeMatchesRepository extends PedigreeMatchesRepository {
  private def pedigreeMatches = Await.result(ReactiveMongoPlugin.database.map(_.collection[JSONCollection]("pedigreeMatches")), Duration(10, SECONDS))
  ...
  override def countMatches(search: PedigreeMatchCardSearch): Future[Int] = {
    val query = Json.obj(
      ...
      "pipeline" ->
        getMatchesQuery(search).++(List(Json.obj("$group" -> Json.obj("_id" -> "0", "count" -> Json.obj("$sum" -> 1)))))
    )

    val runner = Command.run(JSONSerializationPack, FailoverStrategy.default)
    runner.apply(pedigreeMatches.db, runner.rawCommand(query))
          .one[BSONDocument](ReadPreference.nearest)
          .map { bson => {
            val j = Json.toJson(bson)
            (j \ "result").as[Seq[Count]].headOption.fold(0)(_.count)
          } }
  }
  ...
  override def getMatchById(matchingId: String): Future[Option[PedigreeMatchResult]] = {
    pedigreeMatches.find(Json.obj("_id" -> Json.obj("$oid" -> matchingId))).one[PedigreeMatchResult]
  }

  override def getMatchByPedigree(pedigreeId: Long): Future[Option[PedigreeMatch]] = {
  val listaPedigree = pedigreeMatches.find(Json.obj("pedigree.idPedigree" -> pedigreeId)).sort(
      Json.obj("matchingDate" -> -1)).cursor[PedigreeMatchResult]().collect[Seq](Int.MaxValue,Cursor.FailOnError[Seq[PedigreeMatchResult]]())
      listaPedigree.map(_.headOption.map(x=> PedigreeMatch(Left(pedigreeId),x.matchingDate,0,x.pedigree.assignee)))

  }

  override def getMatchByProfile(globalCode: String): Future[Option[PedigreeMatch]] = {
    val listaPedigree = pedigreeMatches.find(Json.obj("profile.globalCode" -> globalCode)).sort(
      Json.obj("matchingDate" -> -1)).cursor[PedigreeMatchResult]().collect[Seq](Int.MaxValue,Cursor.FailOnError[Seq[PedigreeMatchResult]]())
    listaPedigree.map(_.headOption.map(x=> PedigreeMatch(Right(globalCode),x.matchingDate,0,x.pedigree.assignee)))
   ...
   def getMejorLrPedigree(pedigreeId: Long): Future[Option[MatchCardMejorLrPed]] = {
    val listaPedigree = pedigreeMatches.find(Json.obj("pedigree.idPedigree" -> pedigreeId)).sort(
      Json.obj("compatibility" -> -1)).cursor[PedigreeCompatibilityMatch]().collect[Seq](Int.MaxValue,Cursor.FailOnError[Seq[PedigreeCompatibilityMatch]]())
    listaPedigree.map(_.headOption.map(x=> MatchCardMejorLrPed(x.profile.globalCode.text,x.profile.globalCode.text,
      Some(x.profile.categoryId.text),x.compatibility,x.profile.status)))
  ...
  def getMejorLrProf(globalCode: String): Future[Option[MatchCardMejorLrPed]] = {
    val listaPedigree = pedigreeMatches.find(Json.obj("profile.globalCode" -> globalCode)).sort(
      Json.obj("compatibility" -> -1)).cursor[PedigreeCompatibilityMatch]().collect[Seq](Int.MaxValue,Cursor.FailOnError[Seq[PedigreeCompatibilityMatch]]())
    listaPedigree.map(_.headOption.map(x=> MatchCardMejorLrPed(x.pedigree.idPedigree.toString,x.pedigree.idPedigree.toString,
      Some(x.pedigree.caseType),x.compatibility,x.profile.status)))

  }

  override def getTypeCourtCasePedigree(pedigreeId: Long): Future[Option[String]] = {
    val lista = pedigreeMatches.find(Json.obj("pedigree.idPedigree" -> pedigreeId)).sort(
      Json.obj("matchingDate" -> -1)).cursor[PedigreeMatchResult]().collect[Seq](Int.MaxValue,Cursor.FailOnError[Seq[PedigreeMatchResult]]())

    lista.map(_.headOption.map(x => x.pedigree.caseType))
  }

  override def allMatchesDiscarded(pedigreeId: Long): Future[Boolean] = {
    val notDeleted = Json.obj("$nin" -> List(MatchStatus.discarded.toString, MatchStatus.deleted.toString))
    val query = Json.obj("pedigree.idPedigree" -> pedigreeId,
      "pedigree.status" -> notDeleted,
      "profile.status" -> notDeleted)

    pedigreeMatches.count(Some(query)).map { n => n == 0 }
  }
  ...
  override def profileNumberOfPendingMatches(globalCode: String): Future[Int] = {
    val query = Json.obj("profile.globalCode" -> globalCode,
      "pedigree.status" -> MatchStatus.pending.toString,
      "profile.status" -> MatchStatus.pending.toString)
    pedigreeMatches.count(Some(query))
  }
...
override def  countProfilesHitPedigrees(globalCodes:String):Future[Int]={
  val query = Json.obj("profile.globalCode" -> globalCodes,
  ...
  override def profileNumberOfPendingMatchesInPedigrees(globalCode: String,pedigreesIds:Seq[Long]): Future[Int] = {
    val query = Json.obj("profile.globalCode" -> globalCode,
      "pedigree.status" -> MatchStatus.pending.toString,
      "profile.status" -> MatchStatus.pending.toString,
  ...
  private def getMatchesByGroupQuery(search: PedigreeMatchGroupSearch) = {
    val userQuery = if (!search.isSuperUser) {
      Json.obj("$or" -> Json.arr(
        Json.obj("profile.assignee" -> search.user, "profile.status" -> Json.obj("$ne" -> MatchStatus.deleted.toString)),
        Json.obj("pedigree.assignee" -> search.user, "pedigree.status" -> Json.obj("$ne" -> MatchStatus.deleted.toString))
      ))
    } else {
      Json.obj("$or" -> Json.arr(
    ...
    Json.obj("$and" -> Json.arr(userQuery, kindQuery, groupByQuery, status,idCaso))
  }
  ...
  override def getMatchesByGroupPedigree(search: PedigreeMatchGroupSearch): Future[List[MatchCardPed]] = {
   ...
   var ped= pedigreeMatches
      ...
      ped.map { x => x.map { pd => MatchCardPed(pd.profile.globalCode.text, pd.profile.globalCode.text, pd.profile.categoryId.text, pd.compatibility, pd.profile.status,pd.pedigree.assignee, pd.matchingDate.date,pd._id,pd.pedigree.unknown,pd.mtProfile, pd.matchingId) } }
    }else{
      ped.map { x => x.map { pd => MatchCardPed(pd.pedigree.idPedigree.toString, pd.pedigree.idCourtCase.toString,pd.pedigree.caseType, pd.compatibility, pd.profile.status,pd.pedigree.assignee, pd.matchingDate.date,pd._id,pd.pedigree.unknown,pd.mtProfile,pd.matchingId,pd.pedigree.idPedigree.toString) } }
    }
  ...
  private def discardMatch(matchId: String, side: String) = {
    ...
    val modify: JsObject = Json.obj("$set" -> Json.obj(s"$side.status" -> MatchStatus.discarded.toString))
  ...
  override def deleteMatches(idPedigree: Long): Future[Either[String, Long]] = {
    ...
    val modify: JsObject = Json.obj("$set" -> Json.obj("profile.status" -> MatchStatus.deleted.toString,
  ...
  override def getAllMatchNonDiscardedByGroup(id: String, group: String): Future[Seq[PedigreeMatchResult]] = {
    val notDeleted = Json.obj("$nin" -> List(MatchStatus.discarded.toString, MatchStatus.deleted.toString, MatchStatus.hit.toString))
    group match {
      case "pedigree" => {
