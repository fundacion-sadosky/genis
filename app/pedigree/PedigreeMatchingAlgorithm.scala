...
object PedigreeMatchingAlgorithm {
  ...
  def findCompatibilityMatches(frequencyTable: BayesianNetwork.FrequencyTable, p: Profile, pG: PedigreeGenotypification,
    ...
    def toMarkerPlainCPT(mc: MarkerPlainCPT2): (String, PlainCPT) =
      mc.marker -> PlainCPT(mc.cpt.header, mc.cpt.matrix.iterator, mc.cpt.matrix.length)
    val genotypificationH2 = pedigreeGenotypification.genotypificationH2.map(_.map(toMarkerPlainCPT))
    val genotypificationWithMarkers = pedigreeGenotypification.genotypificationWithMarkers.map(toMarkerPlainCPT)
    val lr = BayesianNetwork.getLR(pedigreeGenotypification.unknowns, profile, frequencyTable, analysisType, genotypification, mutationModelType, mutationModelData, n, maxExclusionsAllowed, strictGenotypification, genotypificationH2, genotypificationWithMarkers)
