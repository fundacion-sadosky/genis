...
""")
  ...
  private val findMatchesByProfileFunc = (
    ...
    try {
      ...
      val screeningMatchesToDelete =
        scala.collection.mutable.Set.empty[MatchResultScreening]
      val pedigreeRDDs: Seq[RDD[PedigreeMatchResult]] = pedigrees.map {
        pedigree =>
          ...
          val mutationModelType: Option[Long] = if (mutationModel.nonEmpty) Some(mutationModel.get.mutationType) else None
          val mutationModelData = Await.result(mutationService.getMutationModelData(mutationModel,markers),duration)
          val n = BayesianNetwork.getNFromTable(frequencyTables(completePedigree.get.frequencyTable.get))
          val maxExclusionsAllowed = completePedigree.get.maxMendelianExclusions.getOrElse(
            Await.result(pedigreeMatchingParameterRepository.getMaxMendelianExclusions, duration)
          )
          val caseType = completePedigree.get.caseType
          val pedigreeGenotypificationRDD: RDD[PedigreeGenotypification] =
            Await.result(
              pedigreeGenotypificationRepository.get(pedigreeId),
              duration
            ) match {
              case Some(pg) => Spark2.context.parallelize(Seq(pg), 1)
              case None =>
                logger.warn(
                  s"El pedigri $pedigreeId esta activo pero no tiene " +
                  s"genotipificacion; se omite del matching del perfil " +
                  s"${profile.globalCode.text}"
                )
                Spark2.context.emptyRDD[PedigreeGenotypification]
            }
          val analysisType = Await
            .result(
          ...
          val (mtProfileCode, mithocondrialMatches) =
            if (executeScreeningMitochondrial && isMt) {
              MatchingAlgorithm
                ...
                case Some(mtProfile) =>
                  ...
                  (mtProfile.globalCode.text, screening)
                case None =>
                  (
                    "",
                    (
                      Set.empty[MatchResultScreening],
                      Set.empty[MatchResultScreening]
                    )
                  )
              }
          ...
          screeningMatchesToDelete ++= mithocondrialMatches._1
      ...
      deleteNotUsedScreeningMatches(matchesRDD, screeningMatchesToDelete.toSet)
    } finally {
      matchStatusService.pushJobStatus(PedigreeMatchJobEnded)
    }
  }
