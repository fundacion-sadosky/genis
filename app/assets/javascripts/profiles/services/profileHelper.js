...
function ProfileHelper() {
    ...
    var getMtValues = function(item) {
        ...
        return { pos: pos, base: base ,originalBase:originalBase,tipo:tipo,inputOriginalBase:inputOriginalBase};
    };
	...
	return {
				...
				if(isValid){
                    ...
                    if(range===undefined || range.length !== 2) {
						...
						if(_.uniq(variations).length !== variations.length){
                            ...
                            var outOfRange = variations.filter(function(a) {
                            ...
                            if (outOfRange.length > 0) {
                                ...
                                var invalidVariation = variations.filter(function(a) {
                                    ...
                                    if(mtVals.tipo==="SUS"){
                                        return (mtVals.inputOriginalBase !== mtVals.originalBase) || _.isEmpty(mtVals.base) ||
        ...
        defaultMarkers: function() {
            var defaultValues = {};
            var keys = Object.keys(appConf).filter(function(k) { return k.endsWith("RANGE"); });
            keys.forEach(function(k) { defaultValues[k] = appConf[k].map(function(a) { return a.toString(); }); });
