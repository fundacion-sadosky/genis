define(['angular','lodash','jquery'], function(angular,_,$) {
	'use strict';
	function PedigreeMatchesCourtCaseController($scope, pedigreeMatchesService, matchesService,alertService,profiledataService, $route, pedigreeService,pedigreeMatchesGroupsService, userService) {
        ...
        $scope.discard = function(confirm, jsonMatch){
            ...
            var match = $scope.matches.filter(function(m) { return m.matchCardPed.filter(function (card) {
            return card.id.$oid === JSON.parse(jsonMatch).id.$oid;})[0]; })[0];
