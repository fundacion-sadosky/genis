define(['angular', 'angular', 'jquery', 'lodash'], function (ng, angular, $, _) {
  ...
  var OriginalCtrl = function ($scope, pedigreeService, $log, $routeParams, $modal, $route, alertService, $timeout, hotkeys) {
    ...
    function existsAlias(node) {
      return $scope._pedigree.genogram.some(function (n) {
    ...
    $scope.addNewScenario = function (unknown, profile, matchId) {
      ...
      var ukNode = nodes.filter(function (n) {
        return n.alias === unknown;
      })[0];
      ukNode.globalCode = profile;
      var scenario = {
        pedigreeId: $scope.pedigreeId,
        genogram: nodes,
    ...
    $scope.changeNode = function (node) {
      ...
      if ($scope.isNodeValid(node)) {
        var fathers =
          $scope.pedigree.genogram.filter(function (i) {
            return $scope.isMale(i);
          }).map(function (i) {
            return i.alias;
          });
        ...
        var mothers =
          $scope.pedigree.genogram.filter(function (i) {
            return $scope.isFemale(i);
          }).map(function (i) {
            return i.alias;
          });

        $scope.pedigree.genogram.forEach(function (n) {
          if (fathers.indexOf(n.idFather) === -1) {
            n.idFather = undefined;
          }
          if (mothers.indexOf(n.idMother) === -1) {
    ...
    $scope.deactivate = function () {
      ...
      pedigreeService.canEdit($scope.pedigreeId).then(function (response) {
        ...
        } else if ($scope.scenarios.filter(function (s) {
    ...
    $scope.createScenario = function () {
      ...
      var scenario = {
        ...
        genogram: nodes,
        frequencyTable: $scope.pedigree.frequencyTable,
        pedigreeStatus: $scope.pedigree.status,
        mutationModelId: $scope.pedigree.mutationModelId
      };
      $scope.addScenario(scenario);
    };
    ...
    function getSelectedNodes() {
      var nodes = [];
      var selected = angular.copy($scope.pedigree.genogram.filter(function (n) {
        return n.selected;
      }));
      ...
      var aliases = selected.map(function (n) {
        return n.alias;
      });
    ...
    $scope.noSelected = function () {
      return !$scope.pedigree ||
        $scope.pedigree.genogram.filter(function (n) {
          return n.selected;
        }).length === 0;
    };
    ...
    $scope.removeNodeBoton = function () {
      if ($scope.isEditable()) {
        $scope.guardo = false;
        var selected = $scope.pedigree.genogram.filter(function (n) {
          return n.selected;
        });
        selected.forEach(function (n) {
          if ($scope.pedigree.genogram.some(
