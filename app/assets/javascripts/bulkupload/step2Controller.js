...
//						alert('Hubo un error: ' + response.data);
        ...
        // New function to toggle all replicate checkboxes
        $scope.toggleReplicateAll = function(batchId) {
            if (!$scope.protoProfiles[batchId]) {
                return;
            }

            var allReplicated = $scope.areAllReplicated(batchId);

            $scope.protoProfiles[batchId].forEach(function(r) {
                if (r.status === 'Approved' && !r.replicateDisabled) {
                    r.replicate = !allReplicated; // Toggle replicate value
                }
            });
        };

        // New function to check if all replicates are true
        $scope.areAllReplicated = function(batchId) {
            if (!$scope.protoProfiles[batchId]) {
                return false;
            }
            return $scope.protoProfiles[batchId].every(function(r) {
                return !(r.status === 'Approved' && !r.replicateDisabled) || r.replicate;
            });
        };
