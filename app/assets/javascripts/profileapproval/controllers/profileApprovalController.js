define(['lodash'], function (_) {
    ...
    function ProfileApprovalController(
        ...
        $scope.approveSelected = function () {
            ...
            profileApprovalService.approveProfiles(request, userName).then(function () {
        ...
        $scope.doReject = function (profile, res) {
            ...
            var userName = userService.getUser().name; // Get the username
            var profileModifiedData = $scope.isProfileModified(profile.globalCode);
            var isCategoryModification = !!profileModifiedData;
            profileApprovalService.rejectPendingProfile(profile.globalCode, res, userName, isCategoryModification).then(function () {
        ...
        function getProfilesWithDifferentCategory(profiles) {
            var promises = profiles.map(function(profile) {
                return profileService.findByCode(profile.globalCode)
                    .then(function(response) {
                        var serverProfile = response.data;
                        if (serverProfile.categoryId !== profile.category) {
                            // Store in profilesModified keyed by globalCode
                            $scope.profilesModified[profile.globalCode] = {
                                oldCategory: serverProfile.categoryId,
                                newCategory: profile.category
                            };
                        }
                    })
                    .catch(function(error) {
                        console.error("Error fetching profile data for " + profile.globalCode + ":", error);
                    });
            });
    ...
    function ProfileApprovalController(
        ...
        function getProfilesWithDifferentCategory(profiles) {
            var promises = profiles.map(function(profile) {
                return profileService.findByCode(profile.globalCode)
                    .then(function(response) {
                        return {serverProfile: response.data, clientProfile: profile}; // Return an object containing both profiles
                    })
                    .catch(function(error) {
                        console.error("Error fetching profile data for " + profile.globalCode + ":", error);
            ...
            $q.all(promises).then(function(results) { // Use $q.all
                // Filter out any null results from rejected promises
                results = results.filter(function(result) {
                    return result !== null;
                });

                var categoryInfo = results
                    .filter(function(result) {
                        return result.serverProfile.categoryId !== result.clientProfile.category;
                    })
                    .map(function(result) {
                        return [
                            result.serverProfile.globalCode,
                            {
                                "oldCategory": result.serverProfile.categoryId,
                                "newCategory": result.clientProfile.category
                            }
                        ];
                    });

                $scope.profilesModified = _.fromPairs(categoryInfo);
                $scope.$applyAsync(); // Use $applyAsync for better performance
            }).catch(function(error) {
                console.error("Error in getProfilesWithDifferentCategory:", error);
            });
        }
        ...
        $scope.getCatergoryModificationText = function(globalCode) {
            console.log("getCatergoryModificationText called with globalCode:", globalCode);
            var newCategory = $scope.getModifiedCategory(globalCode, "newCategory");
            var oldCategory = $scope.getModifiedCategory(globalCode, "oldCategory");
            return $.i18n.t('superiorInstanceModifiedCategory', {oldCategory:oldCategory, newCategory:newCategory});
        };
