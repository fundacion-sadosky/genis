...
class CategoriesTest extends PdgSpec with MockitoSugar with Results {
  ...
  "Categories controller" must {
    "return a JSON sturcture with categories, success status code and valid headers" in {
      ...
      val categoriesController = new Categories(mockCategoryService, mockProfileService, mockProfileDataService)
    ...
    "list mappings" in {
      ...
      val target = new Categories(categoryService, mock[ProfileService], mock[ProfileDataService])
  ...
  it must {
    "return a JSON sturcture with subcategories, success status code and valid headers" in {
      ...
      val target = new Categories(mockCategoryService, mock[ProfileService], mock[ProfileDataService])
