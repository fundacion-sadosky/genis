...
import kits.{FullStrKit, StrKit, StrKitRepository, StrKitServiceImpl}
...
class StrKitServiceImplTest extends AnyWordSpec with Matchers with MockitoSugar:
  ...
  private val fullKit = FullStrKit("PP16", "PowerPlex 16", 1, 16, 15, Seq.empty, Seq.empty)
  ...
  "StrKitServiceImpl" must {
    "delegate get() to repository" in {
      ...
      val service = new StrKitServiceImpl(repo, stubMessagesApi())
    ...
    "delegate add() to repository" in {
      val repo = mock[StrKitRepository]
      when(repo.addFull(fullKit)).thenReturn(Future.successful(Right("PP16")))
      val service = new StrKitServiceImpl(repo, stubMessagesApi())

      Await.result(service.add(fullKit), timeout) mustBe Right("PP16")
      verify(repo).addFull(fullKit)
    }
