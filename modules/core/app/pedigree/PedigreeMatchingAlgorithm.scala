...
import types.{AlphanumericId, MongoDate}

import scala.collection.Seq
import profile.GenotypificationByType.GenotypificationByType

/**
 * Migrated from legacy pedigree.PedigreeMatchingAlgorithm.
 * Pure algorithm object — no I/O dependencies.
 */
object PedigreeMatchingAlgorithm:

  val logger: Logger = Logger(this.getClass())

  def matchToPedigreeMatch(
    ...
    matchResult.map { mr =>
      ...
      PedigreeDirectLinkMatch(mr._id, mr.matchingDate, mr.`type`, profile, pedigree, mr.result)
    }
  ...
  def performMatch(
    ...
    val matchResult = MatchingAlgorithm.performMatch(config, p, q, matchingRule, mtRcrs, id, 0, locusRangeMap)
  ...
  def filterGenotipification(
    pG: PedigreeGenotypification,
    markers: List[String]
  ): PedigreeGenotypification =
    pG.copy(genotypification = pG.genotypification.filter(plainCPT =>
      plainCPT.header.find(marker =>
        markers.find(marker2 => marker.contains(marker2)).isDefined
      ).isDefined
    ))
  ...
  def findCompatibilityMatches(
    ...
    val genotypification    = pedigreeGenotypification.genotypification
      .map(plainCPT2 => PlainCPT(plainCPT2.header, plainCPT2.matrix.iterator, plainCPT2.matrix.length))
    val lr = BayesianNetwork.getLR(
    ...
    if lr._1 >= pedigreeGenotypification.boundary then
      ...
      Seq(PedigreeCompatibilityMatch(oid, MongoDate(new java.util.Date()), analysisType.id, matchingProfile, matchingPedigree, lr._1, message = lr._2))
    else Seq.empty
