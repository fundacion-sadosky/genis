...
import matching.{MatchingProfile, MongoId, NewMatchingResult}
...
object PedigreeMatchResult:
  ...
  implicit val writes: OWrites[PedigreeMatchResult] = new OWrites[PedigreeMatchResult]:
    def writes(mr: PedigreeMatchResult): JsObject = mr match
      case p: PedigreeDirectLinkMatch    => Json.toJson(p)(summon[OFormat[PedigreeDirectLinkMatch]]).as[JsObject]
      case p: PedigreeCompatibilityMatch => Json.toJson(p)(summon[OFormat[PedigreeCompatibilityMatch]]).as[JsObject]
      case p: PedigreeMissingInfoMatch   => Json.toJson(p)(summon[OFormat[PedigreeMissingInfoMatch]]).as[JsObject]
  ...
  implicit val reads: Reads[PedigreeMatchResult] = new Reads[PedigreeMatchResult]:
    def reads(js: JsValue): JsResult[PedigreeMatchResult] =
      (js \ "kind").as[PedigreeMatchKind.Value] match
        case PedigreeMatchKind.DirectLink    => Json.fromJson(js)(summon[OFormat[PedigreeDirectLinkMatch]])
        case PedigreeMatchKind.Compatibility => Json.fromJson(js)(summon[OFormat[PedigreeCompatibilityMatch]])
        case PedigreeMatchKind.MissingInfo   => Json.fromJson(js)(summon[OFormat[PedigreeMissingInfoMatch]])
