...
object Tables {
    ...
    // Country table
    final case class CountryRow(code: String, name: String)
    object CountryRow { def tupled = (CountryRow.apply _).tupled }
    class CountryTable(tag: Tag) extends Table[CountryRow](tag, Some("APP"), "COUNTRY") {
      def code = column[String]("CODE", O.PrimaryKey, O.Length(2, varying = true))
    ...
    // Province table
    final case class ProvinceRow(code: String, name: String, country: String)
    object ProvinceRow { def tupled = (ProvinceRow.apply _).tupled }
    class ProvinceTable(tag: Tag) extends Table[ProvinceRow](tag, Some("APP"), "PROVINCE") {
      def code    = column[String]("CODE",    O.PrimaryKey, O.Length(2, varying = true))
      def name    = column[String]("NAME",    O.Length(50, varying = true))
      def country = column[String]("COUNTRY", O.Length(2, varying = true))
      def * = (code, name, country) <> (ProvinceRow.tupled, ProvinceRow.unapply)
    }
    val provinces = TableQuery[ProvinceTable]
  ...
  // ---------------------------------------------------------------------------
    ...
    // Notification table (tabla APP.NOTIFICATION, compartida con legacy)
    final case class NotificationRow(
      id: Long,
      user: String,
      kind: String,
      creationDate: java.sql.Timestamp,
      updateDate: Option[java.sql.Timestamp] = None,
      flagged: Boolean = false,
      pending: Boolean = true,
      info: String
    )

    class NotificationTable(tag: Tag) extends Table[NotificationRow](tag, Some("APP"), "NOTIFICATION") {
      def id           = column[Long]("ID", O.AutoInc, O.PrimaryKey)
      def user         = column[String]("USER", O.Length(50, varying = true))
      def kind         = column[String]("KIND", O.Length(100, varying = true))
      def creationDate = column[java.sql.Timestamp]("CREATION_DATE")
      def updateDate   = column[Option[java.sql.Timestamp]]("UPDATE_DATE", O.Default(None))
      def flagged      = column[Boolean]("FLAGGED", O.Default(false))
      def pending      = column[Boolean]("PENDING", O.Default(true))
      def info         = column[String]("INFO", O.Length(1024, varying = true))
      def * = (id, user, kind, creationDate, updateDate, flagged, pending, info) <>
        ((NotificationRow.apply _).tupled, NotificationRow.unapply)
    }

    val Notification = TableQuery[NotificationTable]
