...
object PValueCalculator {
  ...
  def getProbability(frequencyTable: FrequencyTable)(marker: Marker, allele: BigDecimal): Option[Double] = {
    frequencyTable.get((marker, allele)) match {
  ...
  def calculateRMP(frequencyTable: FrequencyTable)(mode: MatchingProbabilityCalculationMode)(locus: Marker, alleles: Option[Seq[AlleleValue]]): Option[Double] = {
    alleles match {
      case Some(List(Allele(n))) =>
        ...
        } yield mode.wildcard(p, 0) // Asumir que el otro es dropout porque favorece al acusado
  ...
  def mixmixH3(frequencyTable: FrequencyTable)(mode: MatchingProbabilityCalculationMode)(locus: Marker, mix1: Seq[AlleleValue], victim1: Option[Seq[AlleleValue]], mix2: Seq[AlleleValue], victim2: Option[Seq[AlleleValue]], verbose: Boolean = false): Option[Double] = {
    ...
    val pOfScenario = possibleSuspects map { suspect =>
      ...
      if (rem1.length <= 2 && rem2.length <= 2) {
        ...
        val scenarios = for {
          v1 <- victims1 if victim1.isEmpty || ((victim1.get) diff v1).isEmpty
          v2 <- victims2 if victim2.isEmpty || ((victim2.get) diff v2).isEmpty
        } yield {
          (suspect, v1, v2)
        }
        ...
        if (verbose) {
          ...
          scenarios foreach { sce => println("    S=" + showal(sce._1) + " v1=" + showal(sce._2) + " v2=" + showal(sce._3)) }
        }
        ...
        if (scenarios.isEmpty) {
          ...
          val sumOfP = scenarios.foldLeft(0.0) { (prev, current) =>
            ...
            val pVictim1 = if (unconditionatedAlleles1.nonEmpty) {
            ...
            val pVictim2 = if (unconditionatedAlleles2.nonEmpty) {
            ...
            if (verbose) println("       p=" + p.product)
            prev + p.product
          }
          if (verbose) println("sumOfP =" + sumOfP)
          Some(sumOfP)
        }
      } else {
        None
    ...
    if (interesting.isEmpty) None else Some(interesting.sum)
  }
  ...
  def mixmixH2(frequencyTable: FrequencyTable)(mode: MatchingProbabilityCalculationMode)(locus: Marker, mix1: Seq[AlleleValue], victim1: Option[Seq[AlleleValue]], verbose: Boolean = false): Option[Double] = {
    ...
    if (verbose) println("ENTER mixmixH2 WITH m1=" + showal(mix1) + " v1=" + showala(victim1))
    ...
    val pOfScenario = possibleSuspects map { suspect =>
      ...
      if (rem1.length <= 2) {
        ...
        if (verbose) {
          ...
          scenarios foreach { sce => println("    S=" + showal(sce._1) + " v=" + showal(sce._2)) }
        }
        ...
        if (scenarios.isEmpty) {
          ...
          val sumOfP = scenarios.foldLeft(0.0) { (prev, current) =>
            ...
            val pVictim = if (unconditionatedAlleles.nonEmpty) {
  ...
  private def getP(locus: Marker, calculatePForLocus: (Marker, BigDecimal) => Option[Double], theAllele: AlleleValue, calculateMinimumFrequency: (Marker) => Option[Double]) = {
    theAllele match {
      case Allele(n)  => calculatePForLocus(locus, n)
      case XY(_)      => Some(1.0)
      case _          => calculateMinimumFrequency(locus)
    }
  }
