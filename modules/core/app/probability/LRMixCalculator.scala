...
object LRMixCalculator {
  ...
  // Structural change: Akka.system.dispatchers.lookup removed (Play 3 removed play.api.libs.concurrent.Akka).
  // calculateLRMix and lrMixMixNuevo now take an implicit ExecutionContext.
  ...
  def pCond(frequencyTable: FrequencyTable, theta: Double)
    ...
    try {
      ...
      case _: Throwable => throw new NoFrequencyException(s"Marker $marker doesn't exist in frequency table")
    }
  }
  ...
  def pRepOut(pOut: Double)(sample: Array[Double], participants: Array[Double])(occurrences: Map[Double, Int]): Double = {
    ...
    val productCommon  = common.foldLeft(1.0)((prev, allele) => prev * (1 - pow(pOut, occurrences(allele))))
    val productDropOut = dropOut.foldLeft(1.0)((prev, allele) => prev * pow(pOut, occurrences(allele)))
    ...
    if (dropIn.isEmpty) {
      ...
      val productDropIn = dropIn.foldLeft(1.0)((prev, allele) => {
        try {
          ...
          case _: Throwable => throw new NoFrequencyException(s"Value $allele for Marker $marker doesn't exist in frequency table")
        }
      })
  ...
  )(implicit ec: ExecutionContext): Future[LRResult] = Future {
  ...
  private def getResultTotal(
    ...
    if (defense._1 > 0) prosecutor._1 / defense._1 else 0.0
  }
  ...
  def getHypothesisLR(
    ...
    val detailed = sample
      ...
      .map({ marker =>
        ...
        try {
          ...
          val typified: Array[Double]    = selectedGenotypification ++ unselectedGenotypification
          ...
          var alleles: Array[Double] = (sampleGenotypification ++ typified).distinct.toArray
          ...
          while (unknowns.hasNext) {
            val u = unknowns.next()
            val (pCondRes: Double, alleleOccurrences: Map[Double, Int]) = pCond(frequencyTableExtended, theta)(marker, u, typified, selectedOccurrences, unselectedOccurrences)
    ...
    val total =
      if (detailed.values.flatten.exists(_ > 0)) {
        detailed.values.foldLeft(1.0)((prev, current) =>
  ...
  // Structural change: convertSingleAlele inlined from legacy matching.MatchingAlgorithm (not migrated)
  def convertSingleAlele(a: AlleleValue, range: AleleRange): AlleleValue = a match {
    ...
    case MicroVariant(x) if BigDecimal(x + 1) <= range.min               => OutOfLadderAllele(range.min, "<")
    case MicroVariant(x) if BigDecimal(x) > range.max                    => OutOfLadderAllele(range.max, ">")
    case _                                                                => a
  }

  def convertAllelesValues(alleles: Array[AlleleValue], marker: Profile.Marker, allelesRanges: Option[NewMatchingResult.AlleleMatchRange] = None): Array[AlleleValue] = {
    allelesRanges match {
  ...
  def convertAleles(alleles: Array[AlleleValue], locus: Profile.Marker, locusRangeMap: NewMatchingResult.AlleleMatchRange): Array[AlleleValue] = {
    val range = locusRangeMap.get(locus).getOrElse(AleleRange(0, 99))
    alleles.map(a => convertSingleAlele(a, range))
  }

  def flattenGenotypifications(marker: Marker, genotypifications: Array[Genotypification], allelesRanges: Option[NewMatchingResult.AlleleMatchRange] = None): Array[Double] =
  ...
  def lrMixMixNuevo(
    mix1: Genotypification, victim1: Option[Genotypification],
    mix2: Genotypification, victim2: Option[Genotypification],
    frequencyTable: FrequencyTable,
    m1Contributors: Int, m2Contributors: Int,
    allelesRanges: Option[NewMatchingResult.AlleleMatchRange] = None,
    theta: Double, dropIn: Double, dropOut: Double
  )(implicit ec: ExecutionContext): LRResult = {
  ...
  def addFrequencyOfDefault(defaultAllele: BigDecimal, observedAlleles: Array[Double], marker: Marker, frequencyTable: FrequencyTable): FrequencyTable = {
    ...
    val notObservedAllelesProbabilities = frequencyTable.filter(entry =>
      entry._1._1.equals(marker) && !observedAlleles.contains(entry._1._2.toDouble) && entry._1._2 != BigDecimal(-1)
    ).map(_._2).toList
  ...
  def getLRMixMixHypotesis1(
    ...
    val detailed = markers.toArray.map({ marker =>
      try {
        ...
        val cantRandomMenMix1 = m1Contributors - (if (victim1.isEmpty) 0 else 1) - 1
        val cantRandomMenMix2 = m2Contributors - (if (victim2.isEmpty) 0 else 1) - 1
        var alleles: Array[Double] = (mix1Genotipification ++ mix2Genotipification).distinct.toArray
        alleles = alleles :+ 666.0
        val frequencyTableExtended = addFrequencyOfDefault(666.0, alleles, marker, frequencyTable)
        ...
        while (posiblesX.hasNext) {
          ...
          while (posiblesYMix1.hasNext) {
            ...
            while (posiblesYMix2.hasNext) {
              ...
              val (probCond, _) = pCondMixMix(frequencyTableExtended, theta)(marker, (Y1jk ++ Y2jl), (known1 ++ known2 ++ Xj), knownsOccurrences, Map.empty)
          ...
          val (probCondXj, _) = pCondMixMix(frequencyTableExtended, theta)(marker, known1 ++ known2 ++ Xj, Array.empty, Map.empty, Map.empty)
        ...
        case _: NoFrequencyException => (marker, None)
      }
    }).toMap

    val total =
      if (detailed.values.flatten.exists(_ > 0))
        detailed.values.foldLeft(1.0)((prev, current) => if (current.isDefined && current.get != 0) prev * current.get else prev)
      else 0.0
  ...
  def calculateRepetitionFactor(genotypes: Array[Array[Double]]): Double = {
    ...
    genotypes.foreach(genotype => {
      ...
      factor = factor * (unknownsNum / unknownsDenom)
    })
    factor
  }

  def getLRMixMixHypotesis0(
    markers: Set[Marker],
    mix1: Genotypification, victim1: Option[Genotypification],
    mix2: Genotypification, victim2: Option[Genotypification],
    frequencyTable: FrequencyTable,
    ...
    val detailed = markers.toArray.map({ marker =>
      try {
        ...
        while (posiblesYMix1.hasNext) {
          ...
          val (probCondM1: Double, allelesOcurrences: Map[Double, Int]) = pCondMixMix(frequencyTableExtended, theta)(marker, known1 ++ Y1jk, Array.empty, Map.empty, Map.empty)
        ...
        while (posiblesYMix2.hasNext) {
          ...
          val (probCondM2: Double, allelesOcurrences: Map[Double, Int]) = pCondMixMix(frequencyTableExtended, theta)(marker, known2 ++ Y2jk, Array.empty, Map.empty, Map.empty)
