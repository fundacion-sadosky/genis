...
import probability.PValueCalculator.*
import profile.Profile.Genotypification
import profile.{Allele, AlleleValue}
import types.SampleCode

case class GroupSpec(groupLabel: String, theta: Double, frequencyTable: FrequencyTable)
...
object MixHypothesisParticipant {
  implicit val f: Format[MixHypothesisParticipant] = Json.format[MixHypothesisParticipant]
}
...
object MixtureHypothesis {
  implicit val f: Format[MixtureHypothesis] = Json.format[MixtureHypothesis]
}
...
object MixtureLRCalculator {
  ...
  def w(locus: profile.Profile.Marker, c: Seq[AlleleValue], specs: Seq[MixParticipantLocus], groupSpecs: Seq[GroupSpec],
    ...
    groups.foldLeft[Option[Double]](Some(1.0)) { (prod, group) =>
      val numberOfUnknowns = specsByGroup.get(group).get.count { spec => spec.mayBeAlleles.isEmpty }
  ...
  def pmixGivenForLocus(locus: profile.Profile.Marker, groupSpecs: Seq[GroupSpec], mixValues: Seq[AlleleValue], specs: Seq[MixParticipantLocus], observed: Seq[ObservedLoci]) = {
    ...
    if (isInvalid)
      ...
      unknownValues.subsets.map(subset => mixValues.toSet -- subset).foldLeft[Option[Double]](Some(0.0)) { (sum, c) =>
        ...
        for {
          w <- this.w(locus, c.toList, specs, groupSpecs, observed)
          s <- sum
        } yield s + sign * w
      }
    }
  ...
  def pmixGivenH(mixProfile: Genotypification, groupSpecs: Seq[GroupSpec], participants: Seq[MixParticipant], observed: Seq[ObservedProfile]): Map[profile.Profile.Marker, Double] = {
  ...
  def lrMixMix(mix1: Genotypification, victim1: Option[Genotypification], mix2: Genotypification, victim2: Option[Genotypification], frequencyTable: FrequencyTable, mode: MatchingProbabilityCalculationMode): Map[profile.Profile.Marker, Option[Double]] = {

    val loci = mix1.keySet intersect mix2.keySet

    def mixmixH2ForTable = PValueCalculator.mixmixH2(frequencyTable)(mode) _
    def mixmixH3ForTable = PValueCalculator.mixmixH3(frequencyTable)(mode) _

    loci.map { locus =>
      val p3locus  = mixmixH3ForTable(locus, mix1(locus), victim1.flatMap { v => v.get(locus) }, mix2(locus), victim2.flatMap { v => v.get(locus) }, false)
      val p2alocus = mixmixH2ForTable(locus, mix1(locus), victim1.flatMap { v => v.get(locus) }, false)
      val p2blocus = mixmixH2ForTable(locus, mix2(locus), victim2.flatMap { v => v.get(locus) }, false)
