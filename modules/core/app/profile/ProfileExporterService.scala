...
@Singleton
class ProfileExporterServiceImpl @Inject()(
...
)(implicit ec: ExecutionContext) extends ProfileExporterService {
  ...
  override def getFileOf(user: String): File = {
    new java.io.File(s"$exportProfilesPath$user${File.separator}GENisExport.zip")
  }
}
