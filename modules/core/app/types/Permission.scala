...
object Permission {
  ...
  given permissionWrites: Writes[Permission] = (permission: Permission) => JsString(permission.toString)

  given permissionReads: Reads[Permission] = {
    case JsString(s) =>
      Permission.fromString(s) match {
        case Some(p) => JsSuccess(p)
        case None => JsError(s"Unknown permission: $s")
      }
