  ...
  function PedigreeMatchesController(
    ...
    $timeout
      ...
      $scope.status = [
        ...
        {label: "pendientes", value: "pending", index: 1},
        { label: "confirmados",value: "hit", index: 2 },
        {label: "descartados", value: "discarded", index: 3}
      ];
        ...
        $scope.findMatchesForSummary = function(filters) {
          ...
          var getPedigrees = function() {
            var courtCases = $scope
              .matchesSumm
              .flatMap(function(x){return x.matchCardPed.map(function(x) {return x.courtCaseId;});})
          ...
          var buildCourtCaseMap = function(response) {
            ...
            .flatMap(function(x){return x.data;})
            .map(
              function(x) {
          ...
          var collectProfiles = function() {
            ...
            .flatMap(function(x) {return x.genogram;})
              ...
              .flatMap(
                function (x){
                  return x.matchCardPed.map(
                    function(y){return [x.matchCard, y];}
                  );
                }
              )
              .flatMap(
                function(x) {
                  ...
                  .filter(function(y) {return y._id === x[1].idPedigree;})
                  .map(
                    function(y){
                      return [
                        x[0],
                        ...
                        $scope.profileChunks(
                          replaceUnknownInGenomgram(y.genogram, x[0].id)
                            .map(function(x){return [x.globalCode, x];})
                        )
                      ];
                    }
                  );
